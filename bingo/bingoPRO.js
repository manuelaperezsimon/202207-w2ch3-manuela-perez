...
function introducingRandomNumbersInBingoCard() {
  ...
  while (bingoCard.length < 15) {
    ...
    const numbersRepeated = bingoCard.filter(
      (bingoNum) => bingoNum === randomNumber
    );
...
function RandomNumbersRepeatedInBombo() {
  ...
  while (randomNumbersBombo.length < 90) {
    ...
    const randomNumbersRepeated = randomNumbersBombo.filter(
      (randomNumberBombo) => randomNumberBombo === randomNumber
    );
...
function askChangeOrNotBingoCard() {
  const wantToChange = prompt(
    "¿Do you want to change this Bingo Card? Yes/No. If you put another answer we will consider it to start playing."
  );
...
function compareRandomNumberInBomboWithNumberInBingoCard(
  bingoCard,
  currentBomboNumber
) {
  let isAMatch = false;
  bingoCard = bingoCard.map((element, index) => {
  ...
  if (isAMatch) {
    alert(
      `The number that came out is ${currentBomboNumber} and there was a MATCH.`
    );
  } else {
    alert(
      `The number that came out is ${currentBomboNumber} and there was NO a match.`
    );
  }
...
function askKeepPlaying() {
  ...
  const confirmKeepPlaying = confirm("¿Do you want to keep playing?");
...
function showRankig(infoPlayers) {
  infoPlayers.sort((a, b) => b.points - a.points);
...
function executeBingo() {
  ...
  const wantToChange = askChangeOrNotBingoCard();
  ...
  while (confirmKeepPlaying && bingo !== true) {
    compareRandomNumberInBomboWithNumberInBingoCard(
      bingoCard,
      randomNumbersBombo[index]
    );

    const linea = hasLine(bingoCard);
  ...
  const playersPoints = totalPoints - counter;
